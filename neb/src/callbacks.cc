...
int neb::callback_acknowledgement(int callback_type, void* data) {
  ...
  try {
    ...
    if (ack_data->service_description) {
      engine::service* svc((engine::service*)ack_data->object_ptr);
      ack->host_id = svc->get_host()->get_id();
      ack->service_id = svc->get_id();
      ...
      engine::host* hst((engine::host*)ack_data->object_ptr);
      ack->host_id = hst->get_id();
      if (ack->host_id == 0)
...
int neb::callback_comment(int callback_type, void* data) {
  ...
  try {
    ...
    if (comment_data->service_description) {
      engine::service* svc((engine::service*)comment_data->object_ptr);
      comment->host_id = svc->get_host()->get_id();
      comment->service_id = svc->get_id();
      ...
      engine::host* hst((engine::host*)comment_data->object_ptr);
      comment->host_id = hst->get_id();
  ...
  try {
    ...
    if (cvar && cvar->var_name && cvar->var_value) {
      ...
      // Service custom variable.
        ...
        if (svc && svc->get_id() && svc->get_host()->get_id()) {
          misc::shared_ptr<custom_variable>
            new_cvar(new custom_variable);
          new_cvar->enabled = true;
          new_cvar->host_id = svc->get_host()->get_id();
          new_cvar->modified = false;
          new_cvar->name = cvar->var_name;
          new_cvar->service_id = svc->get_id();
          new_cvar->var_type = 1;
          new_cvar->update_time = cvar->timestamp.tv_sec;
          new_cvar->value = cvar->var_value;
          new_cvar->default_value = cvar->var_value;
        ...
        if (svc && svc->get_id() && svc->get_host()->get_id()) {
          misc::shared_ptr<custom_variable>
            old_cvar(new custom_variable);
          old_cvar->enabled = false;
          old_cvar->host_id = svc->get_host()->get_id();
          old_cvar->modified = true;
          old_cvar->name = cvar->var_name;
          old_cvar->service_id = svc->get_id();
          old_cvar->var_type = 1;
          old_cvar->update_time = cvar->timestamp.tv_sec;
...
int neb::callback_event_handler(int callback_type, void* data) {
  ...
  try {
    ...
    if (event_handler_data->service_description) {
      engine::service* svc(static_cast<engine::service*>(
                             event_handler_data->object_ptr));
      event_handler->host_id = svc->get_host()->get_id();
      event_handler->service_id = svc->get_id();
      ...
      engine::host* hst(static_cast<engine::host*>(
                          event_handler_data->object_ptr));
      event_handler->host_id = hst->get_id();
...
int neb::callback_flapping_status(int callback_type, void* data) {
  ...
  try {
    ...
    if (flapping_data->service_description) {
      engine::service* svc(static_cast<engine::service*>(
                             flapping_data->object_ptr));
      flapping_status->host_id = svc->get_host()->get_id();
      flapping_status->service_id = svc->get_id();
      ...
      engine::host* hst(static_cast<engine::host*>(
                          flapping_data->object_ptr));
      flapping_status->host_id = hst->get_id();
    ...
    if ((NEBTYPE_HOSTGROUP_ADD == group_data->type)
      ...
      new_hg->enabled
        = ((group_data->type != NEBTYPE_HOSTGROUP_DELETE)
           && !host_group->get_members().empty());
      new_hg->name = host_group->get_name().c_str();
    ...
    // Service group.
      ...
      new_sg->enabled
        = ((group_data->type != NEBTYPE_SERVICEGROUP_DELETE)
           && !service_group->get_members().empty());
      new_sg->name = service_group->get_name().c_str();
...
int neb::callback_group_member(int callback_type, void* data) {
  ...
  try {
    ...
    if ((member_data->type == NEBTYPE_HOSTGROUPMEMBER_ADD)
      ...
      hgm->group_name = hg->get_name().c_str();
      hgm->poller_id = config::applier::state::instance().poller_id();
      if (hst->get_id() != 0 && hgm->group_id != 0) {
        hgm->host_id = hst->get_id();
    ...
    // Service group member.
      ...
      sgm->group_name = sg->get_name().c_str();
      sgm->poller_id = config::applier::state::instance().poller_id();
      sgm->host_id = svc->get_host()->get_id();
      sgm->service_id = svc->get_id();
...
int neb::callback_host(int callback_type, void* data) {
  ...
  try {
    ...
    // Set host parameters.
    my_host->acknowledged = h->is_acknowledged();
    my_host->acknowledgement_type = h->get_acknowledgement_type();
    my_host->action_url = h->get_action_url().c_str();
    my_host->active_checks_enabled = h->get_active_checks_enabled();
    my_host->address = h->get_address().c_str();
    ...
    my_host->current_state = (h->get_has_been_checked()
                              ? h->get_current_state()
                              : 4); // Pending state.
    my_host->default_active_checks_enabled = h->get_active_checks_enabled();
    ...
    my_host->default_passive_checks_enabled = h->get_passive_checks_enabled();
    my_host->downtime_depth = h->get_scheduled_downtime_depth();
    my_host->display_name = h->get_display_name().c_str();
    ...
    my_host->flap_detection_on_unreachable
      = h->get_flap_detection_on_unreachable();
    my_host->flap_detection_on_up = h->get_flap_detection_on_up();
    ...
    my_host->host_id = h->get_id();
    my_host->host_name = h->get_name().c_str();
    my_host->icon_image = h->get_icon_image().c_str();
    my_host->icon_image_alt = h->get_icon_image_alt().c_str();
    my_host->is_flapping = h->get_flapping();
    ...
    my_host->notify_on_down = h->get_notify_on(
      com::centreon::engine::host::ON_DOWN);
    my_host->notify_on_downtime = h->get_notify_on(
      com::centreon::engine::host::ON_DOWNTIME);
    my_host->notify_on_flapping = h->get_notify_on(
      com::centreon::engine::host::ON_FLAPPING);
    my_host->notify_on_recovery = h->get_notify_on(
      com::centreon::engine::host::ON_RECOVERY);
    my_host->notify_on_unreachable = h->get_notify_on(
      com::centreon::engine::host::ON_UNREACHABLE);
    my_host->obsess_over = h->get_ocp_enabled();
    if (!h->get_output().empty()) {
      my_host->output = h->get_output().c_str();
      my_host->output.append("\n");
    }
    my_host->output.append(h->get_long_output().c_str());
    my_host->passive_checks_enabled = h->get_passive_checks_enabled();
    my_host->percent_state_change = h->get_percent_state_change();
    my_host->perf_data = h->get_perfdata().c_str();
    ...
    my_host->stalk_on_up = h->get_stalk_on_up();
    my_host->state_type = (h->get_has_been_checked()
                           ? h->get_current_state_type()
                           : HARD_STATE);
    my_host->statusmap_image = h->get_statusmap_image().c_str();
    my_host->timezone = h->get_timezone().c_str();
    ...
    if (my_host->host_id != 0) {
      ...
      for (com::centreon::engine::customvar_set::const_iterator
        ...
        if (it->second.get_name() != "HOST_ID") {
          ...
          data.var_name = it->second.get_name().c_str();
          data.var_value = it->second.get_value().c_str();
...
int neb::callback_host_check(int callback_type, void* data) {
  ...
  try {
    ...
    if (hcdata->command_line) {
      host_check->active_checks_enabled = h->get_active_checks_enabled();
      host_check->check_type = hcdata->check_type;
      host_check->command_line = hcdata->command_line;
      if (!hcdata->host_name)
        throw (exceptions::msg() << "unnamed host");
      host_check->host_id = h->get_id();
...
int neb::callback_host_status(int callback_type, void* data) {
  ...
  try {
    ...
    host_status->acknowledged = h->is_acknowledged();
    ...
    // XXX host_status->event_handler = h->event_handler;
    ...
    if (h->get_name().empty())
      throw (exceptions::msg() << "unnamed host");
    {
      host_status->host_id = h->get_id();
      if (host_status->host_id == 0)
        throw (exceptions::msg() << "could not find ID of host '"
               << h->get_name() << "'");
    }
    host_status->is_flapping = h->get_flapping();
    ...
    host_status->obsess_over = h->get_ocp_enabled();
    if (!h->get_output().empty()) {
      host_status->output = h->get_output().c_str();
      host_status->output.append("\n");
    }
    host_status->output.append(h->get_long_output().c_str());
  ...
  try {
    ...
    if ((NEBTYPE_PARENT_ADD == relation->type)
      ...
      if (relation->hst
          ...
          && !relation->dep_svc) {
        // Find host IDs.
        int host_id(relation->dep_hst->get_id());
        int parent_id(relation->hst->get_id());
...
int neb::callback_service(int callback_type, void* data) {
  ...
  try {
    ...
    // Fill output var.
    my_service->acknowledged = s->is_acknowledged();
    my_service->acknowledgement_type = s->get_acknowledgement_type();
    my_service->action_url = s->get_action_url().c_str();
    ...
    my_service->current_state = (s->get_has_been_checked()
                                 ? s->get_current_state()
                                 : 4); // Pending state.
    my_service->default_active_checks_enabled = s->get_active_checks_enabled();
    ...
    my_service->display_name = s->get_display_name().c_str();
    my_service->enabled
    ...
    my_service->flap_detection_on_unknown = s->get_flap_detection_on_unknown();
    my_service->flap_detection_on_warning = s->get_flap_detection_on_warning();
    ...
    my_service->host_id = s->get_host()->get_id();
    my_service->host_name = s->get_host_name().c_str();
    my_service->icon_image = s->get_icon_image().c_str();
    my_service->icon_image_alt = s->get_icon_image_alt().c_str();
    my_service->is_flapping = s->get_flapping();
    my_service->is_volatile = s->get_volatile();
    ...
    my_service->notify_on_critical = s->get_notify_on(
      com::centreon::engine::service::ON_CRITICAL);
    my_service->notify_on_downtime = s->get_notify_on(
      com::centreon::engine::service::ON_DOWNTIME);
    my_service->notify_on_flapping = s->get_notify_on(
      com::centreon::engine::service::ON_FLAPPING);
    my_service->notify_on_recovery = s->get_notify_on(
      com::centreon::engine::service::ON_RECOVERY);
    my_service->notify_on_unknown = s->get_notify_on(
      com::centreon::engine::service::ON_UNKNOWN);
    my_service->notify_on_warning = s->get_notify_on(
      com::centreon::engine::service::ON_WARNING);
    my_service->obsess_over = s->get_ocp_enabled();
    if (!s->get_output().empty()) {
      my_service->output = s->get_output().c_str();
      my_service->output.append("\n");
    }
    my_service->output.append(s->get_long_output().c_str());
    my_service->passive_checks_enabled
      = s->get_passive_checks_enabled();
    my_service->percent_state_change = s->get_percent_state_change();
    my_service->perf_data = s->get_perfdata().c_str();
    ...
    my_service->stalk_on_critical = s->get_stalk_on_critical();
    my_service->stalk_on_ok = s->get_stalk_on_ok();
    my_service->stalk_on_unknown = s->get_stalk_on_unknown();
    my_service->stalk_on_warning = s->get_stalk_on_warning();
    my_service->state_type = (s->get_has_been_checked()
                              ? s->get_current_state_type()
                              : HARD_STATE);
    ...
    if (my_service->host_id && my_service->service_id) {
      ...
      logging::error(logging::medium)
        ...
        << (!s->get_host_name().empty() ? s->get_host_name() : "(unknown)")
        << "', service '"
...
int neb::callback_service_check(int callback_type, void* data) {
  ...
  try {
    ...
    if (scdata->command_line) {
      service_check->active_checks_enabled = s->get_active_checks_enabled();
      ...
      service_check->host_id = s->get_host()->get_id();
      service_check->service_id = s->get_id();
...
int neb::callback_service_status(int callback_type, void* data) {
  ...
  try {
    ...
    service_status->acknowledged = s->is_acknowledged();
    ...
    // XXX service_status->event_handler = s->event_handler;
    ...
    service_status->is_flapping = s->get_flapping();
    ...
    service_status->obsess_over = s->get_ocp_enabled();
    if (!s->get_output().empty()) {
      service_status->output = s->get_output().c_str();
      service_status->output.append("\n");
    }
    service_status->output.append(s->get_long_output().c_str());
    ...
    if (s->get_description().empty())
      throw (exceptions::msg() << "unnamed service");
    service_status->host_name = s->get_host_name().c_str();
    service_status->service_description = s->get_description().c_str();
    service_status->host_id = s->get_host()->get_id();
    service_status->service_id = s->get_id();
    if (!service_status->host_id || !service_status->service_id)
      throw (exceptions::msg() << "could not find ID of service ('"
             << s->get_host_name() << "', '" << s->get_description() << "')");
